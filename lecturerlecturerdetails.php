...
<body>
	...
	<div class="wrapper d-flex align-items-stretch">
		...
		<div id="content" class="p-4 p-md-5 pt-5">
			<?php
					...
					if(mysqli_affected_rows($conn)>0) {
						for($i = 0; $i < mysqli_num_rows($result); $i++){
							...
							$lecid = $row['lec_id'];
							$lecname = $row['lec_name'];
							echo "<center><h2 class=mb-4'>$lecname</h2></center><br>";
						}
					}
				?>
			<p>Below are the information of <?php echo $lecname?>. </p>
			<h5>Personal Information</h5>
			<table class="table table-sm table-sm">
				<tr>
					<th width="15%">Lecturer ID</th>
					<th width="25%">Lecturer Name</th>
					<th width="30%">Lecturer Email</th>
			...
			<br>
			<h5>Intake Incharging</h5>
			<table class="table table-sm table-sm">
				<tr>
					<th width="15%">Intake ID</th>
					<th width="70%">Intake Name</th>
					<th width="15%">Intake Date</th>
				</tr>
				<?php
					$conn = mysqli_connect('localhost:3306', 'root', '', 'onlineassignmentsubmission');
			...
			<br>
			<h5>Module Incharging</h5>
			<table class="table table-sm table-sm">
				<tr>
					<th width="15%">Module ID</th>
					<th width="85%">Module Name</th>
				</tr>
				<?php
					$conn = mysqli_connect('localhost:3306', 'root', '', 'onlineassignmentsubmission');
