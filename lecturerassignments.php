	...
	<div class="wrapper d-flex align-items-stretch">
		...
		<div id="content" class="p-4 p-md-5 pt-5">
			<?php
				...
				$sql = "SELECT n.intake_module_id, n.intake_id, n.module_id, n.lec_id, m.module_id, m.module_name
						FROM intake_module AS n
						INNER JOIN module AS m
						ON n.module_id=m.module_id
						WHERE n.intake_module_id= '$id'";
				$result = mysqli_query($conn, $sql);
				...
				if(mysqli_affected_rows($conn)>0) {
					for($i = 0; $i < mysqli_num_rows($result); $i++){
						...
						echo "<center><h2>$modulename / $moduleid</h2></center>";
					}
				}
			?>
			<p><b>Intake: </b><?php echo $intakeid;?><br>
			Below is the list of assignments of the selected module.</p>
			<button class='btn bg-light'><a href="lectureraddassignment.php?id=<?php echo $id; ?>">Add Assignment</a></button><br><br>
			<?php
				$conn = mysqli_connect('localhost:3306', 'root', '', 'onlineassignmentsubmission');
				...
				if(mysqli_affected_rows($conn)>0) {
					for($i = 0; $i < mysqli_num_rows($result); $i++){
						$row = mysqli_fetch_assoc($result);
						echo '<a href="lecturerassignmentdetails.php?id='.$row['asg_id'].'"><div class="card">';
						echo '<h4><b>'.$row['asg_name'].'</b></h4>';
						echo '<p><b>Hand Out Date: </b>'.$row['asg_start_date_time'].'<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Hand In Date: </b>'.$row['asg_end_date_time'].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a style="float:right;" href="lecturerdeleteassignment.php?id='.$row['asg_id'].'"><b>Delete Assignment</b></p>';
						echo '</div></a><br>';
					}
				}
			?>
