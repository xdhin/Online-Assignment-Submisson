...
<head>
	...
	<title>Assignments | APU Online Assignment Submission System</title>
	<meta charset="utf-8">
...
<body>
	...
	<div class="wrapper d-flex align-items-stretch">
		...
		<div id="content" class="p-4 p-md-5 pt-5">
			<center><h2 class="mb-4">Assignments</h2></center>
			...
			<?php
				...
				$sql = "SELECT a.asg_id, a.asg_name, a.asg_end_date_time, a.intake_module_id, i.intake_module_id, i.intake_id, i.module_id, m.module_id, m.module_name
						FROM assignment AS a
						INNER JOIN intake_module AS i
						ON a.intake_module_id = i.intake_module_id
						INNER JOIN module AS m
						ON i.module_id = m.module_id
						WHERE i.intake_id='$stdintake'
						ORDER BY a.asg_end_date_time DESC";
				$result = mysqli_query($conn, $sql);
				...
				if(mysqli_affected_rows($conn)>0) {
					for($i = 0; $i < mysqli_num_rows($result); $i++){
						...
						echo '<p><b>Due Date: </b>'.$row['asg_end_date_time'].'<br><b>Module: </b>'.$row['module_name'].'</p>';
						echo '</a></div><br>';
					}
				}
			?>
