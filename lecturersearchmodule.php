...
<body>
	...
	<div class="wrapper d-flex align-items-stretch">
		...
		<div id="content" class="p-4 p-md-5 pt-5">
			...
			<form action="lecturersearchmodule.php" method="POST">
				<input type="text" name="search_key" placeholder="Please enter the Module Name to search the Module you want."/>
				<input type="submit" value="Search" /> 
			</form>
			<p>This is the list of module that you are incharging.</p>
			<?php
				$conn = mysqli_connect('localhost:3306', 'root', '', 'onlineassignmentsubmission');
				...
				if ($search_key == NULL) {
					...
					echo '<script>window.location.href = "lecturermodules.php";</script>';
				}
				else{
				}
				...
				$sql="SELECT *
						FROM module 
						WHERE module_leader = $lecid AND module_name LIKE'%". 
				...
				while($row = mysqli_fetch_array($result)){
					...
					echo '<h5><a href="lecturermoduledetail.php?id='.$row['module_name'].'">'.$row['module_name'].'</a></h5>';
					echo '<h6>'.$row['module_id'].'</h6>';
					echo '</div><br>';
				}
			?>
		</div>
