...
<head>
	...
	<title>Add Module to <?php echo $intakeid;?> | APU Online Assignment Submission System</title>
	<meta charset="utf-8">
...
<body>
	...
	<div class="wrapper d-flex align-items-stretch">
		...
		<div id="content" class="p-4 p-md-5 pt-5">
			<center><h2 class="mb-4">Add Module To <?php echo $intakeid;?></h2></center>
			<div class="container">
				<?php
					$conn = mysqli_connect('localhost:3306', 'root', '', 'onlineassignmentsubmission');
					...
					echo "<form method='POST' action='adminaddintakemoduledtb.php?id=$intakeid'>";
				?>
				
					<label for="moduleid">Module ID & Name</label>
					<?php
						$conn = new mysqli('localhost:3306', 'root', '', 'onlineassignmentsubmission') 
						or die ('Cannot connect to db');
						$result = $conn->query("SELECT module_id, module_name from module");
							echo '<select name="module" required>';
					...
					<label for="lecturer">Incharge Lecturer</label>
					...
					<?php
						...
						$result = $conn->query("SELECT lec_id, lec_name from lecturer");
							...
							echo "<option value=''>--Select the Incharge Lecturer--</option>";
							while ($row = $result->fetch_assoc()) {
								unset($id, $name);
								$id = $row['lec_id'];
								$name = $row['lec_name']; 
					...
					<h6><a href="adminintakedetails.php?id=<?php echo $intakeid ?>">Back to Previous Page</a></h6></center></center>
				</form>
			</div>
		</div>
	</div>
